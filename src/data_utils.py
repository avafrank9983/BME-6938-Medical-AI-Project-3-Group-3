...
def prepare_lstm_data(tokenizer, dataset, max_length=128, batch_size=32):
    ...
    return (
        make_loader(train_texts, train_labels, shuffle=True),
        make_loader(val_texts, val_labels),
        make_loader(test_texts, test_labels),
    )

def prepare_transformer_data(tokenizer, dataset, max_length, debug=False):

    train_ds = dataset["train"]
    val_ds = dataset["validation"]
    test_ds = dataset["test"]

    # Debug mode → reduce dataset size for CPU
    if debug:
        train_ds = train_ds.select(range(min(2000, len(train_ds))))
        val_ds = val_ds.select(range(min(500, len(val_ds))))
        test_ds = test_ds.select(range(min(500, len(test_ds))))
